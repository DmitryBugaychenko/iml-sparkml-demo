...
/**
 * Simple example of REST service with Synapse ML Spark Serving
 */
object Main extends {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Define schema for the input JSON. The string
    ...
    // Define input stream using continous streaming config
    val listener: DataFrame = spark
      .readStream
      .option("name", "my_server")
      .continuousServer
      .address("localhost", 8898, "my_api")
      .load()
      .parseRequest("my_api", StructType.fromDDL(schema))

    // Load LightGBM model (the model is trained and stored in unit test)
    val model = PipelineModel.load("./models/lightGbm")

    // Attach model to the stream
    val outputs = model.transform(listener).makeReply("prediction")

    // Configure output
    val query = outputs.writeStream
      .continuousServer
      ...
      .replyTo("my_server")
      .queryName("my_query")
      .trigger(ContinuousTrigger(10000)) // Interval for checkpoints in milliseconds
      .option("checkpointLocation", "./checkpoints/" + Identifiable.randomUID("lightGbm"))
      .start() // Start all the stuff
